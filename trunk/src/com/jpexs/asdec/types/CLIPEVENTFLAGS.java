...
public class CLIPEVENTFLAGS {
    ...
    public String toString() {
        String ret = "";
        if (clipEventKeyUp) ret += "onClipEvent(keyUp)";
        if (clipEventKeyDown) ret += "onClipEvent(keyDown)";
        if (clipEventMouseUp) ret += "onClipEvent(mouseUp)";
        if (clipEventMouseDown) ret += "onClipEvent(mouseDown)";
        if (clipEventMouseMove) ret += "onClipEvent(mouseMove)";
        if (clipEventUnload) ret += "onClipEvent(unload)";
        if (clipEventEnterFrame) ret += "onClipEvent(enterFrame)";
        if (clipEventLoad) ret += "onClipEvent(load)";
        if (clipEventDragOver) ret += "on(dragOver)";
        if (clipEventRollOut) ret += "on(rollOut)";
        if (clipEventRollOver) ret += "on(rollOver)";
        if (clipEventReleaseOutside) ret += "on(releaseOutside)";
        if (clipEventRelease) ret += "on(release)";
        if (clipEventPress) ret += "on(press)";
        if (clipEventInitialize) ret += "on(initialize)";
        if (clipEventData) ret += "onClipEvent(data)";
        if (clipEventConstruct) ret += "on(construct)";
        if (clipEventKeyPress) ret += "on(keyPress)";
        if (clipEventDragOut) ret += "on(dragOut)";
